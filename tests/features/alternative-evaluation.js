...
async function testAlternativeEvaluation() {
  console.log('Starting Alternative Evaluation Test...');
  ...
  try {
    ...
    // Step 2: Set up a decision with multiple criteria
    console.log('Step 2: Setting up decision');
    
    // Enter decision name 
    await page.waitForSelector('#decisionName');
    await page.type('#decisionName', 'Alternative Evaluation Test');
    console.log('Entered decision name');
    
    // Click Next to move to criteria setup - multiple approaches
    try {
    ...
    // Wait for criteria page to load
    await page.waitForNetworkIdle();
    await sleep(500);
    
    // Step 2a: Add criteria names first
    console.log('Step 2a: Adding criteria names');
    ...
    try {
      ...
      if (addCriteriaBtn) {
        console.log('Found Add Criteria button, adding criteria names via UI');
        
        // Add criteria names (without weights yet)
        for (const criterion of ['Price', 'Quality']) {
          // Click the add button
          await addCriteriaBtn.click();
          await sleep(300);
          
          // Get the latest criterion input field
      ...
      console.log('Error while adding criteria names:', error.message);
    }
    
    await sleep(500);
    await page.screenshot({ path: SCREENSHOT_DIR + '/02a-criteria-names.png' });
    
    // Proceed to weights screen - click Next
    ...
    try {
      ...
      if (weightInputs.length >= 2) {
        ...
        // Set weight for first criterion
        try {
          await weightInputs[0].click();
          await weightInputs[0].type('9');
          console.log('Set weight 9 for Price');
        } catch (error) {
          console.log('Error setting first weight, trying JavaScript approach');
        ...
        // Set weight for second criterion
        try {
          await weightInputs[1].click();
          await weightInputs[1].type('7');
          console.log('Set weight 7 for Quality');
        } catch (error) {
          console.log('Error setting second weight, trying JavaScript approach');
          await page.evaluate((index, value) => {
            const inputs = document.querySelectorAll('input[type="number"]');
            if (inputs[index]) {
              inputs[index].value = value;
    ...
    // Proceed to alternatives definition page - click Next
    console.log('Proceeding to alternatives definition page');
    try {
      // Method 1: Find Next button by text
      let nextClicked = await page.evaluate(() => {
        const buttons = Array.from(document.querySelectorAll('button'));
    ...
    // Step 3a: Define multiple alternatives
    ...
    try {
      ...
      if (addAlternativeBtn) {
        ...
        // Add alternatives
        for (const alternative of ['Option A', 'Option B']) {
          // Click the add button
    ...
    // Proceed to evaluation/rating screen
    ...
    try {
      ...
      if (!nextClicked) {
        ...
        if (primaryButtons.length > 0) {
          await primaryButtons[primaryButtons.length - 1].click();
          console.log('Clicked the last primary button to proceed to evaluation');
          nextClicked = true;
        }
      }
      ...
      console.log('Error while proceeding to evaluation screen:', error.message);
      // Final fallback: try to navigate directly if possible
      try {
    ...
    console.log('Step 3b: Rating alternatives for criteria');
    ...
    try {
      ...
      if (ratingInputs.length > 0) {
        ...
        for (let i = 0; i < Math.min(4, ratingInputs.length); i++) {
          const rating = 7 + (i % 3); // Values between 7-9 for variety
        ...
        if (ratingComponents.length > 0) {
          ...
          for (let i = 0; i < Math.min(4, ratingComponents.length); i++) {
            try {
          ...
          const ratingsApplied = await page.evaluate(() => {
            ...
            ratingElements.forEach((el, i) => {
              const rating = 7 + (i % 3); // Values between 7-9
          ...
          if (ratingsApplied) {
            ...
            console.log('No rating elements found. This step may not be applicable in the current UI.');
          }
        }
      }
    } catch (error) {
    ...
    try {
      ...
      console.log('Error while trying to proceed to results:', error.message);
    }
    ...
    console.log('Step 4: Viewing decision results');
    await page.screenshot({ path: SCREENSHOT_DIR + '/04-decision-results.png' });
    ...
    console.log('Alternative Evaluation Test completed successfully');
  } catch (error) {
